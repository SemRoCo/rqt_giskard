...
class NodeItem(GraphItem):
    ...
    def __init__(self, highlight_level, bounding_box, label, shape, color=None, parent=None, label_pos=None, tooltip=None):
        ...
        # Using Polygon to draw the following using bounding_box
            ...
            self._graphics_item = QGraphicsPolygonItem(octagon_polygon)
        elif shape == 'tripleoctagon':
            rect = bounding_box.getRect()
            inner_fold = 3.0
            extra_inner_fold = inner_fold + 2.0  # Adjust this value as needed for visual spacing

            octagon_polygon = QPolygonF([
                # Outer octagon
                QPointF(rect[0], rect[1] + 3 * rect[3] / 10),
                QPointF(rect[0], rect[1] + 7 * rect[3] / 10),
                QPointF(rect[0] + 3 * rect[2] / 10, rect[1] + rect[3]),
                ...
                QPointF(rect[0] + 7 * rect[2] / 10, rect[1]),
                QPointF(rect[0] + 3 * rect[2] / 10, rect[1]),
                # Second (inner) octagon
                QPointF(rect[0] + inner_fold, rect[1] + 3 * rect[3] / 10 + inner_fold / 2),
                QPointF(rect[0] + inner_fold, rect[1] + 7 * rect[3] / 10 - inner_fold / 2),
                QPointF(rect[0] + 3 * rect[2] / 10 + inner_fold, rect[1] + rect[3] - inner_fold),
                QPointF(rect[0] + 7 * rect[2] / 10 - inner_fold, rect[1] + rect[3] - inner_fold),
                QPointF(rect[0] + rect[2] - inner_fold, rect[1] + 7 * rect[3] / 10 - inner_fold / 2),
                QPointF(rect[0] + rect[2] - inner_fold, rect[1] + 3 * rect[3] / 10 + inner_fold / 2),
                QPointF(rect[0] + 7 * rect[2] / 10 - inner_fold, rect[1] + inner_fold),
                QPointF(rect[0] + 3 * rect[2] / 10 + inner_fold, rect[1] + inner_fold),
                # Third (innermost) octagon
                QPointF(rect[0] + extra_inner_fold, rect[1] + 3 * rect[3] / 10 + extra_inner_fold / 2),
                QPointF(rect[0] + extra_inner_fold, rect[1] + 7 * rect[3] / 10 - extra_inner_fold / 2),
                QPointF(rect[0] + 3 * rect[2] / 10 + extra_inner_fold, rect[1] + rect[3] - extra_inner_fold),
                QPointF(rect[0] + 7 * rect[2] / 10 - extra_inner_fold, rect[1] + rect[3] - extra_inner_fold),
                QPointF(rect[0] + rect[2] - extra_inner_fold, rect[1] + 7 * rect[3] / 10 - extra_inner_fold / 2),
                QPointF(rect[0] + rect[2] - extra_inner_fold, rect[1] + 3 * rect[3] / 10 + extra_inner_fold / 2),
                QPointF(rect[0] + 7 * rect[2] / 10 - extra_inner_fold, rect[1] + extra_inner_fold),
                QPointF(rect[0] + 3 * rect[2] / 10 + extra_inner_fold, rect[1] + extra_inner_fold),
            ])
            self._graphics_item = QGraphicsPolygonItem(octagon_polygon)
